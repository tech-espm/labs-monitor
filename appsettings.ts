...
// pm2 start src/app.js --name=monitor-0 -- pm2port=1234
let pm2port = 0;
if (process.argv) {
	for (let i = 0; i < process.argv.length; i++) {
		if (process.argv[i] && process.argv[i].startsWith("pm2port=")) {
			pm2port = parseInt(process.argv[i].substring(8));
			break;
		}
	}
}
...
export = {
	...
	port: pm2port || parseInt(process.env.app_port as string),
	...
	producao: !!parseInt(process.env.app_producao as string),
	...
	api: {
		prefixoInterno: (process.env.app_api_prefixoInterno as string).split(",").map(x => x.trim()),
		prefixoExterno: (process.env.app_api_prefixoExterno as string).split(",").map(x => x.trim()),
	},
};
